...
class ArucoNodeTF(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("aruco_dictionary_id", "DICT_6X6_250")  # ← 修正！
        self.declare_parameter("image_topic", "/image_raw")
        self.declare_parameter("camera_info_topic", "/camera_info")
        self.declare_parameter("camera_frame", "camera_link")  # ← 修正！デフォルト値を設定
        ...
        self.aruco_parameters.minMarkerPerimeterRate = 0.03  # 小さいマーカーを検出
        ...
        self.aruco_parameters.minDistanceToBorder = 1  # 画像端のマーカーも検出
    ...
    def info_callback(self, info_msg):
        ...
        self.get_logger().info(f"  fx={self.intrinsic_mat[0,0]:.2f}, fy={self.intrinsic_mat[1,1]:.2f}")
        self.get_logger().info(f"  cx={self.intrinsic_mat[0,2]:.2f}, cy={self.intrinsic_mat[1,2]:.2f}")
    ...
    def image_callback(self, img_msg):
        ...
        # ★ camera_frameの扱いを修正
        if self.camera_frame is None or self.camera_frame == '':
        ...
        if rejected is not None and len(rejected) > 0:
            cv2.aruco.drawDetectedMarkers(cv_image, rejected, borderColor=(0, 0, 255))  # 赤色で描画
            self.get_logger().info(f"Rejected candidates visualization added ({len(rejected)} candidates)")
        
        # ★ より詳細なデバッグ情報
        detected_count = len(corners) if corners is not None and len(corners) > 0 else 0
        rejected_count = len(rejected) if rejected is not None and len(rejected) > 0 else 0
        
        if detected_count > 0:
            self.get_logger().info(
                f"✓ Detected {detected_count} markers: IDs={marker_ids.flatten()}")
        else:
            self.get_logger().info(
                f"No markers detected (rejected candidates: {rejected_count})")
        ...
        if marker_ids is not None:
            ...
            for i, marker_id in enumerate(marker_ids):
                ...
                self.get_logger().info(
                    ...
                    f"x={tvecs[i][0][0]:.3f}, y={tvecs[i][0][1]:.3f}, z={tvecs[i][0][2]:.3f}")
