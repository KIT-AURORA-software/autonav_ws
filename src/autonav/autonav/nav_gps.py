...
class NavGPS(Node):
    ...
    def gps_callback(self, msg: NavSatFix):
        ...
        if self.target_lat is None:
            self.target_lat = self.current_latitude + 0.00001
            self.target_lon = self.current_longitude + 0.00001
            self.get_logger().info(
                f"Target set: lat={self.target_lat:.10f}, lon={self.target_lon:.10f}"
            )
