...
class IntegratedGateNavigator(Node):
    def __init__(self):
        ...
        # obstacle
        self.front_obstacle_dist = None
        self.left_min_dist = None
        self.right_min_dist = None
        ...
        # obstacle avoidance
        self.obstacle_detect_dist = 1.0
        self.obstacle_clear_dist = 1.3
        self.avoid_turn_speed = 0.35
        ...
        self.avoid_timeout_sec = 8.0
        self.avoid_blind_start_time = None
        self.avoid_blind_turn_sec = 0.8
        self.avoid_phase = "TURN"   # TURN or FORWARD
        self.avoid_forward_start_time = None
        self.avoid_forward_sec = 1.2
        self.avoid_forward_speed = 0.12
        self.avoid_cooldown_until = None
        self.avoid_cooldown_sec = 1.0
    ...
    def is_obstacle_ahead(self):
        # VISION_NAV 中は回避しない
        if self.state == self.STATE_VISION_NAV:
            return False

        return (
            self.front_obstacle_dist is not None and
            self.front_obstacle_dist < self.obstacle_detect_dist
        )
    ...
    def scan_callback(self, msg: LaserScan):
        ...
        def valid(v):
            return math.isfinite(v) and msg.range_min < v < msg.range_max
        ...
        # 前方 ±15deg
        fi0 = angle_to_index(math.radians(-15.0))
        fi1 = angle_to_index(math.radians(15.0))
        front_indices = range(min(fi0, fi1), max(fi0, fi1) + 1)

        # 左 20〜60deg
        li0 = angle_to_index(math.radians(20.0))
        li1 = angle_to_index(math.radians(60.0))
        left_indices = range(min(li0, li1), max(li0, li1) + 1)

        # 右 -60〜-20deg
        ri0 = angle_to_index(math.radians(-60.0))
        ri1 = angle_to_index(math.radians(-20.0))
        right_indices = range(min(ri0, ri1), max(ri0, ri1) + 1)

        front_vals = [ranges[i] for i in front_indices if valid(ranges[i])]
        left_vals = [ranges[i] for i in left_indices if valid(ranges[i])]
        right_vals = [ranges[i] for i in right_indices if valid(ranges[i])]

        self.front_obstacle_dist = min(front_vals) if front_vals else None
        self.left_min_dist = min(left_vals) if left_vals else None
        self.right_min_dist = min(right_vals) if right_vals else None
    ...
    def control_gps_nav(self, now):
        ...
        if self.is_obstacle_ahead():
            self.prev_state = self.STATE_GPS_NAV
            self.state = self.STATE_AVOID
            self.avoid_start_time = now

            left_score = self.left_min_dist if self.left_min_dist is not None else 999.0
            right_score = self.right_min_dist if self.right_min_dist is not None else 999.0
            self.avoid_direction = 1 if left_score > right_score else -1

            self.get_logger().warn(
                f"[GPS_NAV] obstacle detected: front={self.front_obstacle_dist:.2f}, "
                f"left={left_score:.2f}, right={right_score:.2f}, dir={self.avoid_direction}"
            )
            return
    ...
    def control_avoid(self, now):
        if self.avoid_start_time is None:
            self.avoid_start_time = now

        # 全体タイムアウト
        if (now - self.avoid_start_time) > self.avoid_timeout_sec:
            self.get_logger().warn("[AVOID] timeout -> return")
            self.avoid_cooldown_until = now + self.avoid_cooldown_sec
            self.state = self.prev_state if self.prev_state is not None else self.STATE_GPS_NAV
            self.avoid_direction = 0
            self.avoid_start_time = None
            self.avoid_blind_start_time = None
            self.avoid_forward_start_time = None
        ...
        if self.avoid_phase == "TURN":
            if self.front_obstacle_dist is None:
                if self.avoid_blind_start_time is None:
                    self.avoid_blind_start_time = now

                # 少しだけさらに回る
                if (now - self.avoid_blind_start_time) < self.avoid_blind_turn_sec:
                    self.drive_mode = self.MODE_PIVOT
                    angular_z = self.avoid_direction * self.avoid_turn_speed
                    self.publish_cmd_mode(0.0, angular_z, self.drive_mode)

                    dir_str = "left" if self.avoid_direction > 0 else "right"
                    self.get_logger().info(f"[AVOID-TURN] no front scan, keep turning {dir_str}")
                    return
                ...
                self.avoid_blind_start_time = None
                self.get_logger().info("[AVOID] turn complete -> FORWARD")
                return
            ...
            # frontが十分増えたら前進フェーズへ
            if self.front_obstacle_dist > self.obstacle_clear_dist:
                self.avoid_phase = "FORWARD"
                self.avoid_forward_start_time = now
                self.get_logger().info(
                    f"[AVOID] cleared front={self.front_obstacle_dist:.2f} -> FORWARD"
                )
                return
            ...
            left_log = f"{self.left_min_dist:.2f}" if self.left_min_dist is not None else "None"
            right_log = f"{self.right_min_dist:.2f}" if self.right_min_dist is not None else "None"
            dir_str = "left" if self.avoid_direction > 0 else "right"

            if self.front_obstacle_dist is not None:
                self.get_logger().info(
                    f"[AVOID-TURN] front={self.front_obstacle_dist:.2f}, "
                    f"left={left_log}, right={right_log}, "
                    f"turn={dir_str}, mode={self.drive_mode}"
                )
            return
        ...
        if self.avoid_phase == "FORWARD":
            if self.avoid_forward_start_time is None:
                self.avoid_forward_start_time = now

            elapsed = now - self.avoid_forward_start_time

            if elapsed < self.avoid_forward_sec:
                # 向きはそのまま、少し前進して横に逃げる
                self.drive_mode = self.MODE_OPPOSITE_PHASE
                self.publish_cmd_mode(self.avoid_forward_speed, 0.0, self.drive_mode)
                self.get_logger().info(
                    f"[AVOID-FWD] elapsed={elapsed:.2f}/{self.avoid_forward_sec:.2f}"
                )
                return

            # 前進完了 → GPS_NAVへ戻る
            self.get_logger().info("[AVOID] forward complete -> return")
            self.avoid_cooldown_until = now + self.avoid_cooldown_sec
            self.state = self.prev_state if self.prev_state is not None else self.STATE_GPS_NAV
            self.avoid_direction = 0
            self.avoid_start_time = None
